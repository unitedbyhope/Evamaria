...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, REQ_CHANNEL, CHNL_LNK, GRP_LNK, LOG_CHANNEL
...
NOR_IMG = "https://telegra.ph/file/6253efb388b4bc3381902.jpg"
...
async def next_page(bot, query):
    ...
    btn.insert(0, [
        InlineKeyboardButton(f'🎗 {search} 🎗', callback_data='rheq1')
    ])    
    
    btn.insert(0, [
        InlineKeyboardButton(f'🗂 ꜰɪʟᴇs: {total}', callback_data='req1'),
        InlineKeyboardButton('❗️ɪɴꜰᴏ', callback_data='qinfo')
    ])
    ...
    if n_offset == 0:
        btn.append( 
            [    
             InlineKeyboardButton("⏮ βΔCҜ", callback_data=f"next_{req}_{key}_{off_set}")]
        )
        btn.append(                     
            [             
             InlineKeyboardButton(f"📜 ΠΩ MΩRΣ PΔGΣS 📜",callback_data="pages")]
        )
        
    elif off_set is None:
        btn.append(
            [InlineKeyboardButton(text=f"ΠΣXT ⏭", callback_data=f"next_{req}_{key}_{n_offset}"),
             InlineKeyboardButton(text=f"📕 PΔGΣ: {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages")]
        )
    else:
        btn.append(
            [
                InlineKeyboardButton(f"📗 PΔGΣ ΠUMβΣR: {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pagees")
            ],
        )
        btn.append(
            [
                InlineKeyboardButton("⏮ βΔCҜ", callback_data=f"next_{req}_{key}_{off_set}"),
                InlineKeyboardButton("ΠΣXT ⏭", callback_data=f"next_{req}_{key}_{n_offset}")
            ],
        )
    try:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
          ...
          #  InlineKeyboardButton('ᴍᴏᴠɪᴇs', url='t.me/at3movies'),
        ...
        await query.answer("Fᴇᴛᴄʜɪɴɢ DᴀᴛᴀBᴀꜱᴇ ♻️")
        buttons = [[
            InlineKeyboardButton('ʙᴀᴄᴋ', callback_data='start'),
            InlineKeyboardButton('ʀᴇꜰʀᴇsʜ', callback_data='rfrsh')
        ]]
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        InlineKeyboardButton(f'🎗 {search} 🎗', callback_data='iuuggu')]
    )
    btn.insert(0, [
        InlineKeyboardButton(f'🗂 ꜰɪʟᴇs: {total_results}', callback_data='req1'),
        InlineKeyboardButton('❗️ɪɴꜰᴏ', callback_data='qinfo')]
    )
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"ΠΣXT ⏭", callback_data=f"next_{req}_{key}_{offset}"),
             InlineKeyboardButton(text=f"📕 PΔGΣ: 1/{math.ceil(int(total_results) / 10)}", callback_data="pages")]
        )
    else:
        btn.append(
    ...
    if imdb and imdb.get('poster'):
        try:
         ...
         fmsg = await message.reply_text(text=cap, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(btn))
